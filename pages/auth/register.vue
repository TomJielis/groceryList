    ...
    <div class="bg-white/90 dark:bg-slate-900/90 p-8 rounded-2xl shadow-2xl w-full max-w-sm border border-border-light dark:border-border-dark">
      ...
      <div v-if="!verifyMailMessage">
        <form @submit.prevent="handleRegister" class="space-y-5">
          <div>
            <label for="name" class="block text-sm font-medium text-slate-700 dark:text-slate-100 mb-1">{{ i18n.t('auth.name') }}</label>
            <input
                type="text"
                id="name"
          ...
          <div class="bg-white/80 dark:bg-slate-900/80 rounded-xl shadow p-4 border border-border-light dark:border-border-dark">
            <div class="space-y-3">
          ...
          <button type="submit" class="w-full bg-accent text-white py-3 rounded-xl hover:bg-accent-dark active:scale-95 transition font-semibold shadow border border-accent/80 focus:ring-2 focus:ring-accent">
