...
<template>
  <div class="w-full">
    <!-- Header -->
    <div class="flex items-center gap-2 pb-3 border-b border-surface-200 mb-1">
      <svg class="w-4 h-4 text-surface-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/>
      </svg>
      <span v-if="invites.length > 0" class="text-xs border border-surface-200 px-2 py-0.5 rounded font-medium">
        {{ invites.length }}
      </span>
    </div>
    ...
    <div v-if="loading" class="flex items-center justify-center py-8">
      <div class="animate-spin h-6 w-6 border border-surface-200 border-t-surface-500 rounded-full"></div>
    </div>
    ...
    <div v-else>
      <transition-group name="list" tag="div">
        <div
          ...
          :key="invite.id"
          class="flex items-center justify-between border-b border-surface-200 py-3"
        >
          <div class="flex items-center gap-3 flex-1 min-w-0">
            <!-- Avatar -->
            <div class="w-8 h-8 rounded flex items-center justify-center text-sm font-medium flex-shrink-0">
          ...
          <button
            ...
            class="flex-shrink-0 p-1.5 transition-colors ml-2 rounded"
