  ...
  <div class="flex-1 min-h-0 flex flex-col overflow-y-auto space-y-6">
    ...
    >
      ...
      <div class="bg-slate-100 dark:bg-slate-800 p-3 rounded-xl flex items-center justify-center">
        <img :src="card.attachment" alt="Attachment"
             class="w-full h-auto rounded-xl cursor-pointer shadow-md hover:shadow-xl transition"
    ...
    <div v-if="isModalOpen" class="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 backdrop-blur"
         @click="closeModal">
      <div
          class="bg-white/90 dark:bg-slate-900/90 rounded-2xl p-6 max-w-lg w-full relative shadow-2xl border border-border-light dark:border-border-dark"
          @click.stop>
        <button
